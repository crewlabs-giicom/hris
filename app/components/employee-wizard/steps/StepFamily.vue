...
import type { FamilyItem } from '~/schemas/employee-wizard.schema'

interface FamilyRow extends FamilyItem {
  id: string
}

const props = defineProps<{ employeeId?: string | null }>()

const rows = ref<FamilyRow[]>([])
...
async function load() {
  ...
  loading.value = true
  try {
    const res = await useApi<{ data: FamilyRow[] }>(`/api/v1/employees/${props.employeeId}/family`)
    rows.value = res.data
  } finally {
    loading.value = false
  }
...
async function saveRow(row: FamilyRow) {
  saving.value = row.id
  try {
    await useApi(`/api/v1/employees/${props.employeeId}/family/${row.id}`, {
      method: 'PUT',
      body: {
        name: row.name || undefined,
        birthDate: row.birthDate || undefined,
        familyRelation: row.familyRelation || undefined,
      },
    })
  } catch (e: any) {
...
async function removeRow(row: FamilyRow) {
  try {
    await useApi(`/api/v1/employees/${props.employeeId}/family/${row.id}`, { method: 'DELETE' })
...
<template>
  <UiCardForm>
    <p v-if="!employeeId" class="text-[11px] text-ink-soft mb-3">
      Isi step Identitas dan klik Lanjut dulu supaya Keluarga bisa diisi.
    </p>
    ...
    <div v-for="row in rows" :key="row.id" class="relative border border-line rounded-lg p-3.5 mb-3 bg-[#FBFBFC]">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-2">
        ...
        <UiFormField label="Hubungan Keluarga">
          <UiSelectSearch
