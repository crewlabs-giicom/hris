...
watch([freeAttendances, manualAttendanceType], ([isFree, type]) => {
  ...
  } else if (type === 'Manual Absen' && !isFree) {
    clockIn.value = ''
    clockOut.value = ''
  }
})
...
async function onSubmit() {
  ...
  // If not free attendance, make sure either clockIn or clockOut is filled
  ...
  try {
    await useApi('/api/v1/shift-schedule/manual', {
      method: 'POST',
      body: {
    ...
    toast.success('Pengajuan manual attendance berhasil disimpan')
    router.push('/hr/shift-schedule/manual')
  } catch (err: any) {
    toast.error(err?.data?.statusMessage || 'Gagal menyimpan pengajuan')
  } finally {
...
<template>
  <div @paste="handleFilePaste">
    <!-- Page Header -->
    <UiPageHeader title="Add Manual Attendance" breadcrumb="Dashboard / Shift Schedule / Manual Attendance / New">
      <template #actions>
        <button
          type="button"
