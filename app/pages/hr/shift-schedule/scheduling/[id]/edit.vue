...
async function loadDetails() {
  loading.value = true
  try {
    const res = await useApi<any>(`/api/v1/shift-schedule/scheduling/${id}`)
    const d = res.data
    employeeId.value = String(d.employeeId)
    shiftId.value = String(d.shiftId)
    isFix.value = d.isFix === 1
    isOff.value = d.isOff === 1
    
    // Format date strings to YYYY-MM-DD
    const formatDateStr = (dStr: string) => dStr.split('T')[0]
    validFrom.value = formatDateStr(d.validFrom)
    validTo.value = formatDateStr(d.validTo)
  } catch (err: any) {
    toast.error(err?.data?.statusMessage || 'Gagal memuat detail jadwal shift')
...
async function onSubmit() {
  ...
  try {
    await useApi(`/api/v1/shift-schedule/scheduling/${id}`, {
      method: 'PUT',
      body: {
    ...
    toast.error(err?.data?.statusMessage || 'Gagal memperbarui jadwal shift')
  } finally {
...
<template>
  <div>
    <!-- Page Header -->
    <UiPageHeader :title="`Edit Scheduling #${id}`" :breadcrumb="`Dashboard / Shift Schedule / Edit Scheduling / #${id}`">
      <template #actions>
        <button
          type="button"
    ...
    <div v-if="loading" class="flex justify-center items-center py-24 bg-white border border-line rounded-xl">
      <span class="text-xs text-ink-soft animate-pulse">Memuat data jadwal...</span>
    </div>
