...
async function loadDetails() {
  loading.value = true
  try {
    const res = await useApi<any>(`/api/v1/shift-schedule/adjustment/${id}`)
    const d = res.data
    employeeId.value = String(d.employeeId)
    shiftId.value = String(d.shiftId)
    isOff.value = d.isOff === 1
    status.value = d.status
    adjustmentDate.value = d.adjustmentDate.split('T')[0]
  } catch (err: any) {
    toast.error(err?.data?.statusMessage || 'Gagal memuat detail penyesuaian')
...
async function onSubmit() {
  ...
  try {
    await useApi(`/api/v1/shift-schedule/adjustment/${id}`, {
      method: 'PUT',
      body: {
    ...
    toast.error(err?.data?.statusMessage || 'Gagal memperbarui penyesuaian')
  } finally {
...
<template>
  <div>
    <!-- Page Header -->
    <UiPageHeader :title="`Edit Schedule Adjustment #${id}`" :breadcrumb="`Dashboard / Shift Schedule / Adjustment / Edit / #${id}`">
      <template #actions>
        <button
          type="button"
    ...
    <div v-if="loading" class="flex justify-center items-center py-24 bg-white border border-line rounded-xl">
      <span class="text-xs text-ink-soft animate-pulse">Memuat data penyesuaian...</span>
    </div>
