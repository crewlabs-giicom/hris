...
async function fetchDetail() {
  ...
  try {
    ...
    form.value = {
      ...
      bank: d.bank,
      rekening: d.rekening,
      paymentTo: d.paymentTo,
      financeId: String(d.financeId),
      ...
      items: d.items.map((item: any) => ({
        ...
        manufacturerId: String(item.manufacturerId),
        economicAge: item.economicAge,
        images: item.images,
      })),
    }

    // Populate initial master attachments from database paths
    attachments.value = d.images.map((path: string) => {
...
async function onSubmit() {
  ...
  try {
    await useApi(`/api/v1/asset-requests/${id}`, {
      method: 'PUT',
      body: f,
    })
    toast.success('Pengajuan aset berhasil diperbarui!')
    router.push('/hr/asset-request')
  } catch (e: any) {
...
watch(
  ...
  (newTabs) => {
    ...
    if (activeTab) {
      activeTab.title = 'Edit Asset Request'
    }
  },
...
<template>
  <div @paste="handleFilePaste">
    <!-- Page Header -->
    <UiPageHeader title="Edit Pengajuan Aset" breadcrumb="HR / Asset Requests / Edit">
      <template #actions>
        <button
          type="button"
    ...
    <div v-if="loading" class="py-24 bg-white rounded-xl border border-line flex justify-center items-center my-4">
      <span class="text-xs text-ink-soft animate-pulse">Mengambil rincian data pengajuan...</span>
    </div>
      ...
      <div class="lg:col-span-2 flex flex-col gap-4">
        ...
        <UiCard class="p-6">
          <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
            ...
            <!-- Status & Payment Date (Show only in Edit) -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-line pt-4 mt-2">
              <UiFormField label="Status Pengajuan" required>
                <UiSelectSearch
                  v-model="form.status"
              ...
              <UiFormField label="Payment Date (Paid)">
                <UiDatePicker v-model="form.paymentDate" placeholder="Optional tanggal bayar" />
              </UiFormField>
            </div>
          </form>
        </UiCard>
