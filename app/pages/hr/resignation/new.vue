...
async function onSubmit() {
  ...
  // Filter tasks to ensure they are filled
  ...
  try {
    await useApi('/api/v1/resignation', {
      method: 'POST',
      body: {
        ...
        tasks: validTasks,
        assets: assets.value, // hidden placeholder assets
      },
    })
    toast.success('Pengajuan resign berhasil ditambahkan')
    router.push('/hr/resignation')
  } catch (err: any) {
    toast.error(err?.data?.statusMessage || 'Gagal menyimpan pengajuan resign')
  } finally {
...
<template>
  <div>
    <!-- Page Header -->
    <UiPageHeader title="Add Resignation" breadcrumb="Dashboard / Resignation / New Form">
      <template #actions>
        <button
          type="button"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 my-4">
        ...
        <form @submit.prevent="onSubmit" class="flex flex-col gap-5">
          ...
          <UiCard class="p-6">
            ...
            <div class="flex flex-col gap-4">
              ...
              <!-- Read-only Employee Profile Panel (Shows after select) -->
              ...
              <div v-else-if="employeeDetail" class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-line text-xs">
                ...
                <div class="flex flex-col gap-1">
                  <span class="text-[10px] text-ink-soft uppercase tracking-wider font-semibold">NIK (Nomor Induk Karyawan)</span>
            ...
            <div class="flex flex-col gap-3">
              ...
              >
                ...
                <input
                  ...
                  placeholder="Contoh: Mengembalikan dokumen pertanggungjawaban program..."
          ...
          <!-- Hidden Asset input section (Will be generated from asset table when ready) -->
      ...
      <div class="lg:col-span-1">
        <UiCard class="p-6 bg-[#FFFDFB] border-[#FFEADF]">
          <h3 class="text-xs font-bold text-ink uppercase tracking-wider mb-2 text-[#E07040]">Panduan Resign</h3>
          <ul class="text-xs text-ink-soft leading-relaxed list-disc list-inside flex flex-col gap-2">
            <li>Pilih nama karyawan yang mengajukan pengunduran diri terlebih dahulu.</li>
            <li>Pastikan **Tanggal Resign** (Tanggal Efektif) sudah disepakati bersama manajemen.</li>
            <li>Pilih **Tipe Resign** yang sesuai (misalnya habis kontrak atau pemutusan hubungan kerja).</li>
            <li>Tuliskan deskripsi alasan pengunduran diri secara objektif dan jelas.</li>
            <li>Tambahkan daftar **Tugas & Serah Terima** tanggung jawab berupa dokumen soft copy maupun hard copy agar proses transisi lancar.</li>
          </ul>
        </UiCard>
      </div>
    </div>
