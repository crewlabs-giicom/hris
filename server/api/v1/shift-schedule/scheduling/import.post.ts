...
import { schedules, employees, shifts } from '~~/server/db/schema'
...
export default defineEventHandler(async (event) => {
  ...
  // Fetch employees list to index NIK and EmployeeCode in memory
  const emps = await db
  ...
  // Fetch shifts list to index shift codes
  const shs = await db
  ...
  const inserted = await db.transaction(async (tx) => {
    ...
    for (const r of rows) {
      ...
      if (empId && shId && r.validFrom && r.validTo) {
        await tx.insert(schedules).values({
          employeeId: empId,
          shiftId: shId,
          validFrom: new Date(r.validFrom),
          validTo: new Date(r.validTo),
          isFix: String(r.isFix || '').toLowerCase() === 'yes' ? 1 : 0,
